...
<p>The application integrates user input through a simple, intuitive Streamlit interface, where users are prompted with \
  targeted yes/no and multiple-choice questions. This input is dynamically transformed using pre-trained PCA models and \
    processed through the XGBoost classifier for fraud prediction. The system handles null values, optimizes feature \
      importance for reducing user inputs, and seamlessly incorporates clustering for behavioral and transactional data \
...
disclaimer_content = "The information and tools provided by this application are intended for educational and informational \
  purposes only. The results and predictions generated by the machine learning models should not be used as the sole basis \
    for making financial decisions or assessing risk. Users should exercise caution and consult with financial experts or \
      professionals before acting on the information provided. The developer of this application do not assume any \
        responsibility for any financial losses or damages that may result from the use of this tool. The accuracy \
          of the predictions depends on the quality of the input data and the assumptions made during model training."
...
def transactionDetails(labels, my_address_data, my_address):
  ...
  if st.checkbox("Transaction Address same as Home Address", key="tx_address", value=True):
    ...
    st.markdown("##### From where you have done your transaction?")
...
def cardDetails(labels, TransactionAmt):
  ...
  payment_method = st.selectbox("Have you done the payment using a card?", ['Select One', 'Yes', 'No'], key="payment_method")
  card4, card6 = -999, -999
...
def billingDetails(purchaser_email, card_holder_name, name, phone_no, tx_phone_no, my_address, tx_address, country, tx_country):
  ...
  M1 = st.selectbox("Is the billing address same as the shipping address?", ['Select One', 'Yes', 'No'], key="M1")
  ...
  if M4 == 'Yes':
    M4 = st.selectbox("Have you done the transaction from nearby home location?", ['Select One', 'Yes', 'No'], key="M4_home")
...
def behavioralDetails(TransactionAmt, min_last, max_last, _Hours, my_address_data, tx_address_data):
  ...
  cols = st.columns(3)
  with cols[0]:
    V1 = st.selectbox("Have you recently changed account information?", ['Select One', 'Yes', 'No'], key="V1")
    V1 = 1 if V1 == 'No' else 0
  with cols[1]:
    V14 = st.selectbox("Has there any other fraud scenario with you?", ['Select One', 'Yes', 'No'], key="V14")
  ...
  V94 = st.selectbox("What is the nature of your's purchase history?", ['Select One', 'Mostly small transactions', 'A mix of small and large transactions', 'Mostly large transactions'], key="V94")
  ...
  V35 = st.selectbox("How long has your’s account been active?", ['Select One', 'Less than 1 month', '1-6 months', '6-12 months', 'More than a year'], key="V35")
  V35 = (0 if V35 == 'More than a year' else 1 if V35 == '6-12 months' else 2 if V35 == '1-6 months' else 3) / 3

  V75 = st.selectbox("How often does you change their password?", ['Select One', 'Less than once a year', 'Once a year', 'Every 6 months', 'Every 3 months', 'Monthly'], key="V75")
  ...
  V69 = st.selectbox("How frequently does the user make transactions online?", ['Select One', 'Daily', 'Weekly', 'Bi-weekly', 'Monthly', 'Every few months', 'Rarely'], key="V69")
  ...
  C7 = st.slider("How frequent the small transactions per day?", min_value=0, max_value=100, value=5, step=1, key="C7") / 2256
  C12 = st.slider("How frequent the large transactions per day?", min_value=0, max_value=10, value=2, step=1, key="C12") / 3188

  C6 = st.slider("How frequent the small transactions per week?", min_value=0, max_value=1000, value=30, step=1, key="C6") / 2252
  C14 = st.slider("How frequent the large transactions per week?", min_value=0, max_value=100, value=13, step=1, key="C14") / 1429

  C5 = st.slider("How frequent the small transactions per month?", min_value=0, max_value=10000, value=200, step=1, key="C5") / 376
  C9 = st.slider("How frequent the large transactions per month?", min_value=0, max_value=1000, value=30, step=1, key="C9") / 572
  ...
  D2 = st.slider("How much days difference between the small transactions?", min_value=0, max_value=100, value=2, step=1, key="D2") / 641
  D11 = st.slider("How much days difference between the medium transactions?", min_value=0, max_value=365, value=15, step=1, key="D11") / 936
  D3 = st.slider("How much days difference between the large transactions?", min_value=0, max_value=730, value=40, step=1, key="D3") / 1076
  D5 = st.slider("How much days difference between the very large transactions?", min_value=0, max_value=730, value=80, step=1, key="D5") / 1088
  D4 = st.slider("How much days difference between the exceptional transactions?", min_value=0, max_value=1095, value=200, step=1, key="D4") / 1213
...
def deviceInfo(labels):
  ...
  DeviceType = 1 if DeviceType == 'Desktop' else 2 if DeviceType == 'Mobile' else 3 if DeviceType == 'Tablet' else -999

  # For DeviceInfo
  DeviceInfo = labels[st.selectbox("Device Info", list(labels.keys()), key="DeviceInfo")]

  # For DeviceType, DeviceInfo, id_30, id_31, id_33
...
def app():
  st.write("This is a simple web app to predict whether a transaction is a fraud or not.")  
  ...
  if st.button("Predict"):
    ...
    st.toast("Pre-Processing is started...", icon="⏳")
    with st.spinner('Model is under processing...'):
      progress_bar = st.progress(0)
      for percent_complete in range(100):
          time.sleep(0.03)
          progress_bar.progress(percent_complete + 1)
    data = preprocessing(data)
    st.toast("Pre-Processing is completed...", icon="✅")
    time.sleep(1)
    ...
    with results:
      ...
      with st.spinner('Model is under processing...'):
        ...
        for percent_complete in range(100):
            time.sleep(0.03)
            progress_bar.progress(percent_complete + 1)
      st.toast("Prediction is done...", icon="✅")
      try:
        prediction = predict(data)
        st.write(f'Hello {name}!')
        st.write('Based from the Machine Learning model, the risk of this transaction being a fraud is:')
        if prediction[:,1] >= 0.5:
          st.error("**HIGH**", icon="🚫")
          st.toast("The transaction is classified as a fraud.", icon="🚫")
        else:
          st.success("**LOW**", icon="✅")
          st.toast("The transaction is classified as a non-fraud.", icon="🎉")
        
        st.warning('Disclaimer: **The results from this test are not intended for any financial or legal advice.**\
                  The model was only trained in 590540 data with personal attributes only. Moreover, the analysis of \
                  this model states that it is likely to classify certain attributes such as the transaction amount, \
                  maximum and minimum limit of your transaction, and the distance of the transaction location as high \
                  importance in determining if the transaction is a fraud or not.', icon="⚠️")
        
        st.info('Accuracy: The Machine Learning Model used for prediction was first evaluated in an unknown data \
                consisting of 506691 records, The model correctly classified 89.54% of transactions as fraud and \
                88.66% of transactions as non-fraud. The model has an AUC-ROC score of 0.94. The model takes into \
                consideration the cost of misclassifying a non-fraud transaction as fraud. That is why the model \
                is more likely to classify a transaction as non-fraud than fraud.', icon="ℹ️")
        
        details = st.expander("More Details", expanded=False)
        with details:
          st.write('According to the ML model:')
          st.write('The Probability that it will classify you as a fraud transaction is:')
          st.error(f'**{prediction[:,1][0]:.2%}**')
          st.write('The Probability that it will classify you as a non-fraud transaction is:')
          st.success(f'**{prediction[:,0][0]:.2%}**')
          st.write('**Note:** If the Probability it will classify the transaction as a fraud is over 0.5, then the model will classify it as a fraud transaction.')
...
if __name__ == '__main__':
  st.set_page_config(page_title="Payment Fraud Detection App", page_icon="./static/images/logo/favicon.ico")
  ...
  with title:
    ...
    with col1:
      st.image('./static/images/logo/logo.png', width=100)
    with col2:
  ...
  if st.session_state.button_clicked:
    ...
    with cols[6]:
      st.button('Disgree')
      st.toast("You have to agree to the terms to proceed with the application.", icon="🚨")
